...
> 更新时间：2026-07-21（UTC+8，Goodcase 真图链路验收、批量裁判与 Worker 稳定性修复）

---

## 1. 一句话现状

Prompt Loop（Coze Loop fork）本地栈可跑；Prompt 详情已形成**编排、观测、评测、智能优化**四工作区。智能优化已接通 OptimizeTask IDL、MySQL、真实 API、可选优化模型和带租约的异步 Worker。实验路径已复用原评估器；Goodcase 路径已接 EvaluationSet Items、真实 VLM 图片、原 Prompt baseline 实跑与批量参考答案裁判，并按候选相对 baseline 的增益选优。剩余首要风险是裁判一致性保护。
...
| 最新相关提交 | `f9e9f53a` — `feat(evaluation): complete prompt optimization loop`；本轮 Goodcase 变更尚未提交 |
...
- 根据模式使用 1–3 轮、每轮 2–3 个候选；固定拆分优化集/验证集。Goodcase 先对每个样本执行一次 baseline，所有候选复用该输出，再按验证集 `after-before` 增益选优；最佳增益不超过 0.001 时明确拒绝推荐。调用预算以 24/48/96 为基础，Goodcase 额外计入一次 baseline 样本执行和每候选的 12 条一批裁判；LLM 临时错误最多重试 3 次。
...
1. **Goodcase 裁判一致性保护**：当前批量 LLM Judge 仍可能对输出协议变形或明显拒答误给高分；需增加 JSON/Schema 硬校验、拒答检测、可疑全满分复核，并把校验失败视为 0 分或重试。
2. 实验 evaluator 名称目前报告以 version ID 展示；后续可批量补齐展示名（Goodcase 已显示 `Goodcase Judge`）。
...
批次 `12` 是保守默认值，不是 API 或模型硬限制：它为 4096 输出 token、VLM 大证据和结构化逐 case 返回预留安全余量。下一阶段可按输入/输出 token 预算动态选择约 4–40 条，并在超限、JSON 截断或漏项时二分重试；详细依据见 TRD 7.2。
...
当前任务：先补 Goodcase 裁判一致性保护，再补实验 evaluator 展示名。baseline 对照与无增益拒绝推荐已完成；在硬校验完成前，仍不要把单次 Goodcase 高分直接视为自动发布依据。
...
2026-07-21 智能优化 1–5、Goodcase 双来源、批量裁判、真实多模态 part 注入及 baseline 对照选优均已编译部署；当前镜像为 `localhost/coze-loop:local-goodcase-baseline`，运行容器为 `coze-loop-app`，部署前回滚点为 `coze-loop-app-before-goodcase-baseline-20260721`。MySQL 已应用 `lease_token / lease_expires_at / attempt_count` 和完整来源快照 `source` 列；更早回滚点仍保留。

真实验收盘点：轮胎 Goodcase 版本 `0.1.1` 有 11 条 MultiPart 样本。10 条任务 `7664925583716909057` 验证了 20 次候选调用 + 2 次批量裁判调用，但当时图片仍在 RPC/归一化链路丢失，结果不可作成功凭证。修复后 1 条任务 `7664935468542197761` 成功；两次候选调用的 JoyBuild 组包日志均为 `inline_images=1, file_images=0, inline_bytes=49872`，输出 `image_quality=ok`，证明真图已进入模型。该任务的 Judge 仍把输出协议变形结果评为 1 分，因此在 baseline 已补齐后，下一优先级是裁判硬校验，而不是继续扩大样本量。

baseline P0 验收任务 `7664954798201896961`：账本为 baseline 1 次、候选 2 次、批量裁判 2 次、候选生成 2 次；baseline 与两次候选均携带同一张 `inline_images=1 / inline_bytes=49872` 真图。最终最佳增益为 `-0.1000`，任务按规则拒绝推荐并返回 `no candidate improved the Goodcase baseline`，证明系统不再把绝对高分候选直接当作优化成功。
