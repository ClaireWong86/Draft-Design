...
Prompt Loop（Coze Loop fork）本地栈可跑；智能优化 P0/P1 与动态裁判分批已闭环（含 `no_gain`、协议硬校验、Guard）。**剩余扩展项**：RocketMQ 唤醒、VLM 诊断探针（不阻塞当前体验）。
...
2. RocketMQ 唤醒（任务量上来后再做；MySQL lease 已正确）
3. VLM 诊断探针 / 文本策略路由 / 视觉变换（research P1–P4）
...
P0/P1 与动态裁判分批已完成。剩余扩展：RocketMQ 唤醒、VLM 诊断探针（research）。
