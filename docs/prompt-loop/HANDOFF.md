...
> 更新时间：2026-07-22（UTC+8，P2–P4 MVP：策略路由 / 提示侧视觉变换 / SolutionPlan）

---

## 1. 一句话现状

Prompt Loop（Coze Loop fork）本地栈可跑；智能优化已覆盖 P0–P4 MVP：裁判保护、`no_gain`、动态分批、可选 RMQ 唤醒、确定性探针、按 failure_stage 的文本策略路由、提示侧视觉变换，以及 `SolutionPlan` 候选搜索。**剩余深化**：真图派生媒体（裁剪/SoM 落盘）、裸描述/定位 VLM 探针、成本/延迟多目标护栏。
...
| 最新相关提交 | 见分支 HEAD；含 `no_gain`、动态分批、RMQ 可选唤醒、确定性诊断探针 |
...
2. [x] 协议变形硬校验回归：`scripts/local/regress-protocol-judge.sh`（含「LLM 满分 + 协议破坏 → 硬归零」）
...
1. [x] Goodcase 裁判动态分批 4–40 + 截断/漏项二分重试（TRD 7.2）
2. [x] RocketMQ 可选唤醒（默认 disable；MySQL lease 仍权威；本地 enqueue + best-effort publish）
3. [x] VLM 确定性诊断探针 MVP（输入完整性 / 输出协议 → `failure_modes`）
4. [x] 文本策略路由 + 提示侧视觉变换 + `SolutionPlan` 搜索（research P2–P4 MVP）
...
运维备忘：只用 **`podman-loop-dev`**；一键 `.local-build/redeploy-no-gain-smoke.sh` / `.local-build/regress-no-gain.py` / `scripts/local/regress-protocol-judge.sh`；宿主机 16GB 易 OOM；回滚镜像 `localhost/coze-loop:local-goodcase-baseline`。

候选执行方案仍为方案 1（临时执行适配器，不建持久 Prompt 版本）；契约见 TRD 6.4。

### P2 — 扩展（不阻塞当前体验）

- [x] 批量裁判按 token 预算动态选 4–40 条，超限/截断时二分重试（TRD 7.2）  
- [x] RocketMQ 可选唤醒（`optimize_task_wake_rmq`，默认 `disable_produce/consume: true`；开启前确保 topic 已建）  
- [x] VLM 确定性诊断探针 MVP（输入完整性 / 输出协议；裸描述/定位等 VLM 探针仍待做）  
- [x] 文本策略路由 / 提示侧视觉变换 / `SolutionPlan` 选优（P2–P4 MVP；真图派生媒体与多目标护栏仍待深化）  
...
14. [x] 协议硬校验回归脚本 + Goodcase 裁判动态分批/二分重试
15. [x] RocketMQ 可选唤醒 OptimizeTask（lease 权威；默认关闭）
16. [x] 确定性诊断探针 MVP 写入 diagnosis.failure_modes
17. [x] P2–P4 MVP：failure_stage 文本策略路由、提示侧视觉变换、`SolutionPlan` 搜索与报告落盘
...
P0–P4 MVP 已完成（含策略路由、提示侧视觉变换、SolutionPlan）。深化项：真图派生媒体、VLM 探针、成本/延迟护栏。
不要把单次 Goodcase 高分直接视为自动发布依据；无增益看「未提升」报告。
